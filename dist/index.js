...
const VERSION_TAG = `[MatrixOS v${VERSION}]`;
/**
...
function requireGuideToken() {
    ...
    if (!token) {
        return tagVersion({
            content: [{
                type: 'text',
                text: 'This tool is not available in this version.'
            }],
            isError: true,
        });
    }
...
async function main() {
    ...
    console.error(`MatrixOS MCP server running on stdio (v${VERSION})`);
}
main().catch((error) => {
