...
export async function checkPlugins(rootPath) {
    ...
    const [projectSettings, userSettings, installedPlugins] = await Promise.all([
        readJson(projectSettingsPath),
        readJson(userSettingsPath),
        readJson(installedPluginsPath),
    ]);

    const projectPlugins = getEnabledPlugins(projectSettings);
    const userPlugins = getEnabledPlugins(userSettings);

    // Also read from installed_plugins.json (Claude Code plugin manager format)
    const installedPluginIds = installedPlugins?.plugins
        ? Object.keys(installedPlugins.plugins)
        : [];

    // Merge into unique set
    const allPlugins = [...new Set([...projectPlugins, ...userPlugins, ...installedPluginIds])];
